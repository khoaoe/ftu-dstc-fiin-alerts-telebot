...
def _ensure_numba_stub() -> None:
    ...
    numba_module.njit = _identity
    numba_module.prange = range
    sys.modules.setdefault('numba', numba_module)
...
def _ensure_matplotlib_stub(force: bool = False) -> None:
    ...
    def _subplots(*args, **kwargs):
        rows = kwargs.get("nrows")
        cols = kwargs.get("ncols")
        if rows is None and cols is None and args:
            rows = args[0]
            cols = args[1] if len(args) > 1 else 1
        rows = rows or 1
        cols = cols or 1
        axes = [_Axes() for _ in range(rows * cols)]
        axes_obj = axes if len(axes) > 1 else axes[0]
        return None, axes_obj
